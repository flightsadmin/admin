        ...
        <div class="card">
            <div class="card-header">
                <div class="d-flex align-items-center justify-content-between">
                    <h5>Users</h5>
                    @can('createUser')
                    <button data-bs-toggle="modal" data-bs-target="#dataModal" class="btn btn-sm btn-primary float-end align-items-center bi-person-add">
                        Add User
                    </button>
                    @endcan
                </div>
                        ...
                        <tbody>
                            ...
                            <tr wire:key="{{ $user->id }}">
                                ...
                                <td class="text-center">
                                    <button data-bs-toggle="modal" data-bs-target="#viewModal" class="btn btn-info custom-btn-sm text-white bi-eye" wire:click="edit({{ $user->id }})"> </button>
                                @if($user->id == auth()->user()->id || auth()->user()->can('editUser'))
                                    <button data-bs-toggle="modal" data-bs-target="#dataModal" class="btn btn-primary custom-btn-sm bi-pencil-square" wire:click="edit({{ $user->id }})"> </button>
                                    @can('deleteUser')
